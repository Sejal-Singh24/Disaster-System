...
# ── Cache — avoids hitting API on every request ───────────
...
def get_disaster_type(rainfall_mm, wind_kmh):
    if rainfall_mm > 100:   return "Flash Flood"
    elif rainfall_mm > 60:  return "Riverine Flood"
    elif rainfall_mm > 25:  return "Urban Flood"
    elif wind_kmh > 80:     return "Cyclone"
    else:                   return "None"
...
def gdacs_event_type(eventtype):
    mapping = {
        ...
        "DR": "Drought", "WF": "Wildfire", "VO": "Volcano",
    }
    return mapping.get(eventtype, "Disaster")


# ── Fetch single district weather ────────────────────────
async def fetch_one(client, district, index):
    try:
        ...
        if data.get("cod") == 401:
            print(f"❌ API Key invalid! Check your WEATHER_API_KEY in .env")
            return None

        if data.get("cod") != 200:
            print(f"⚠️ {district['name']}: API error {data.get('cod')} — {data.get('message','')}")
            return None
        ...
        print(f"⚠️ {district['name']} fetch failed: {e}")
        return None



async def fetch_all_weather(client):
    results    = []
    batch_size = 10  # 10 calls at a time
    delay      = 1.5  # 1.5 seconds between batches

    for i in range(0, len(DISTRICTS), batch_size):
        batch   = DISTRICTS[i:i + batch_size]
        tasks   = [fetch_one(client, d, i + j) for j, d in enumerate(batch)]
        batch_r = await asyncio.gather(*tasks)
        results.extend([r for r in batch_r if r is not None])

        # Delay between batches to avoid rate limit
        if i + batch_size < len(DISTRICTS):
            await asyncio.sleep(delay)

    print(f"✅ Weather: {len(results)}/{len(DISTRICTS)} cities fetched")
    return results
...
# ── Main alerts endpoint with caching ────────────────────
@router.get("/")
async def get_alerts():
    ...
    if _cache["data"] and (time.time() - _cache["timestamp"]) < CACHE_TTL:
        print(f"📦 Returning cached alerts ({len(_cache['data'])} items)")
        return _cache["data"]
    ...
    async with httpx.AsyncClient() as client:
        ...
        weather_alerts = await fetch_all_weather(client)
