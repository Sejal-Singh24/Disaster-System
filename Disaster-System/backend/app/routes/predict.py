...
def load_model():
    """Model load karo — M1 ka banaya hua model.pkl"""
    try:
        with open(MODEL_PATH, "rb") as f:
            return pickle.load(f)
    except FileNotFoundError:
        print("⚠️  model.pkl nahi mila — M1 se maango!")
        return None
...
class PredictRequest(BaseModel):
    disaster_type   : str            = Field(..., example="flood",
                                             description="flood/earthquake/cyclone/drought/landslide/wildfire")
    district        : str            = Field(..., example="Moradabad")
    state           : str            = Field(..., example="Uttar Pradesh")
    rainfall_mm     : float          = Field(..., example=250.5, description="Rainfall in mm")
    temperature_c   : float          = Field(..., example=38.2,  description="Temperature in Celsius")
    wind_speed_kmh  : float          = Field(..., example=45.0,  description="Wind speed in km/h")
    population      : int            = Field(..., example=887000, description="District population")
    area_sq_km      : float          = Field(..., example=3493.0, description="District area in sq km")
    river_level_m   : Optional[float]= Field(None, example=8.5,  description="River level in meters")
    magnitude       : Optional[float]= Field(None, example=5.2,  description="Earthquake magnitude")
...
def estimate_people_at_risk(severity_score: float, population: int) -> int:
    """Severity score ke basis pe affected population estimate karo"""
...
def get_recommendations(severity_label: str, disaster_type: str) -> list[str]:
    """Severity aur disaster type ke basis pe recommendations"""
    base = {
        "Critical": [
...
def rule_based_severity(req: PredictRequest) -> float:
    """
    Fallback: jab tak M1 ka model.pkl na aaye
    Simple rule-based severity calculation
    """
    score = 0.0
...
async def predict_disaster(req: PredictRequest):
    """
    Disaster ka impact predict karo.
    M1 ka ML model use karta hai — agar nahi mila toh rule-based fallback.
    """
    ...
    features = np.array([[
        ...
        req.population / max(req.area_sq_km, 1),   # density
    ]])

    # ML model se predict karo (ya fallback)
