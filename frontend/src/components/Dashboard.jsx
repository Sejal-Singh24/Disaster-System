...
const DISASTER_DATA = {
  ...
  earthquake: {
    ...
    stateDeaths : [
      { state: "Gujarat",         deaths: 20005 },
      { state: "Tamil Nadu",      deaths: 10749 },
      { state: "Kashmir",         deaths: 1309  },
      { state: "Sikkim",          deaths: 111   },
      { state: "Manipur",         deaths: 80    },
      { state: "Uttarakhand",     deaths: 72    },
      { state: "Assam",           deaths: 50    },
      { state: "Maharashtra",     deaths: 30    },
    ],
    subtypes : [
  ...
  cyclone: {
    ...
    stateDeaths : [
      { state: "Odisha",       deaths: 12500 },
      { state: "Andhra Pradesh", deaths: 9800 },
      { state: "Tamil Nadu",   deaths: 8200  },
      { state: "West Bengal",  deaths: 6100  },
      { state: "Gujarat",      deaths: 4200  },
      { state: "Maharashtra",  deaths: 2800  },
      { state: "Kerala",       deaths: 1900  },
      { state: "Karnataka",    deaths: 1200  },
    ],
    subtypes : [
  ...
  drought: {
    ...
    stateDeaths : [
      { state: "Rajasthan",    deaths: 800  },
      { state: "Maharashtra",  deaths: 650  },
      { state: "Gujarat",      deaths: 400  },
      { state: "Karnataka",    deaths: 350  },
      { state: "MP",           deaths: 280  },
      { state: "Andhra Pradesh", deaths: 200 },
      { state: "Telangana",    deaths: 180  },
      { state: "Jharkhand",    deaths: 120  },
    ],
    subtypes : [
  ...
  landslide: {
    ...
    stateDeaths : [
      { state: "Uttarakhand",  deaths: 2800 },
      { state: "Kerala",       deaths: 1900 },
      { state: "HP",           deaths: 1500 },
      { state: "J&K",          deaths: 1200 },
      { state: "Assam",        deaths: 980  },
      { state: "Sikkim",       deaths: 650  },
      { state: "Arunachal",    deaths: 420  },
      { state: "Manipur",      deaths: 310  },
    ],
    subtypes : [
  ...
  wildfire: {
    ...
    yearlyData  : [
      ...
      { year: "2002", deaths: 0,  affected_M: 0,    events: 0 },
      { year: "2003", deaths: 0,  affected_M: 0,    events: 0 },
      { year: "2004", deaths: 0,  affected_M: 0,    events: 0 },
      { year: "2005", deaths: 0,  affected_M: 0,    events: 0 },
      { year: "2006", deaths: 0,  affected_M: 0,    events: 0 },
      { year: "2007", deaths: 0,  affected_M: 0,    events: 0 },
      { year: "2008", deaths: 0,  affected_M: 0,    events: 0 },
      ...
      { year: "2013", deaths: 0,  affected_M: 0,    events: 0 },
      { year: "2014", deaths: 0,  affected_M: 0,    events: 0 },
      { year: "2015", deaths: 0,  affected_M: 0,    events: 0 },
    ...
    stateDeaths : [
      { state: "Uttarakhand", deaths: 43  },
      { state: "Odisha",      deaths: 31  },
      { state: "HP",          deaths: 25  },
      { state: "MP",          deaths: 18  },
      { state: "Chhattisgarh",deaths: 12  },
    ],
    subtypes : [
      { type: "Forest Fire",   count: 3, pct: 75 },
      { type: "Grass Fire",    count: 1, pct: 25 },
    ],
    notable : [
  ...
  tsunami: {
    ...
    stateDeaths : [
      { state: "Tamil Nadu",       deaths: 8000  },
      { state: "Andhra Pradesh",   deaths: 1500  },
      { state: "Kerala",           deaths: 500   },
      { state: "Andaman & Nicobar",deaths: 749   },
    ],
    subtypes : [
      { type: "Tectonic Tsunami", count: 2, pct: 100 },
    ],
    notable : [
      { year: "2004", event: "Indian Ocean Tsunami",        deaths: "10,749", affected: "0.65M", sev: "high" },
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: "#7a9bbf", fontFamily: "monospace" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
        {[
          { label: "Total Deaths (2000–21)", value: totalDeaths.toLocaleString(),          icon: "💀", color: "#ff3b5c", sub: "EMDAT recorded"         },
          { label: "People Affected",        value: `${totalAffected.toFixed(1)}M`,        icon: "👥", color: "#ffb347", sub: "total displaced"         },
          { label: `${d.title} Events`,      value: totalEvents,                           icon: d.icon, color: d.color, sub: "India 2000–2021"         },
          { label: "Worst Year Deaths",      value: worstYear.deaths.toLocaleString(),     icon: "⚠️", color: "#ff3b5c", sub: `${worstYear.year} worst` },
          { label: "Avg Events/Year",        value: (totalEvents / 22).toFixed(1),         icon: "📊", color: "#00e676", sub: "per year avg"            },
        ].map((s) => (
          <div key={s.label} className="card" style={{ borderTop: `2px solid ${s.color}`, padding: 16 }}>
            ...
            <div style={{ fontSize: 10, color: "#7a9bbf", fontFamily: "monospace", marginTop: 2 }}>{s.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div className="card">
          <p className="card-title">Annual {d.title} Deaths — India (2000–2021)</p>
          <div style={{ fontSize: 10, fontFamily: "monospace", color: "#7a9bbf", marginBottom: 10 }}>
            📊 EM-DAT · {d.worstNote}
          </div>
          <ResponsiveContainer width="100%" height={210}>
            <BarChart data={d.yearlyData} margin={{ top: 4, right: 8, bottom: 0, left: -10 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e2d45" />
              <XAxis dataKey="year" tick={{ fill: "#7a9bbf", fontSize: 9, fontFamily: "monospace" }} axisLine={false} tickLine={false} interval={2} />
              <YAxis tick={{ fill: "#7a9bbf", fontSize: 10 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={TT} formatter={(val) => [val.toLocaleString(), "Deaths"]} />
              <Bar dataKey="deaths" fill={d.color} radius={[3, 3, 0, 0]} opacity={0.85} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={210}>
            <AreaChart data={d.yearlyData} margin={{ top: 4, right: 8, bottom: 0, left: -10 }}>
              <defs>
              ...
              <XAxis dataKey="year" tick={{ fill: "#7a9bbf", fontSize: 9, fontFamily: "monospace" }} axisLine={false} tickLine={false} interval={2} />
              <YAxis tick={{ fill: "#7a9bbf", fontSize: 10 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={TT} formatter={(v) => [`${v}M`, "Affected"]} />
              <Area type="monotone" dataKey="affected_M" stroke={d.color} strokeWidth={2} fill="url(#affGrad)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div className="card">
          <p className="card-title">{d.title} Events Per Year (2000–2021)</p>
          <div style={{ fontSize: 10, fontFamily: "monospace", color: "#7a9bbf", marginBottom: 10 }}>
            📊 {d.eventsNote}
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={d.yearlyData} margin={{ top: 4, right: 8, bottom: 0, left: -10 }}>
              ...
              <Line type="monotone" dataKey="events" stroke="#00e676" strokeWidth={2} dot={{ r: 3, fill: "#00e676" }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={d.stateDeaths} layout="vertical" margin={{ top: 4, right: 16, bottom: 0, left: 10 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e2d45" />
              <XAxis type="number" tick={{ fill: "#7a9bbf", fontSize: 9 }} axisLine={false} tickLine={false} />
              <YAxis type="category" dataKey="state" tick={{ fill: "#b0c8e0", fontSize: 10, fontFamily: "monospace" }} axisLine={false} tickLine={false} width={80} />
              <Tooltip contentStyle={TT} formatter={(v) => [v.toLocaleString(), "Deaths"]} />
              <Bar dataKey="deaths" fill="#ff6b35" radius={[0, 3, 3, 0]} opacity={0.85} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div className="card">
          <p className="card-title">{d.title} Type Distribution — India EMDAT</p>
          {d.subtypes.map((ft) => (
            <div key={ft.type} style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                <span style={{ fontSize: 13, fontFamily: "monospace", color: "#b0c8e0" }}>{ft.type}</span>
                <span style={{ fontSize: 12, fontWeight: 700, fontFamily: "monospace", color: d.color }}>
                  {ft.count} ({ft.pct}%)
                </span>
              </div>
              <div style={{ background: "#0a0e1a", borderRadius: 4, height: 8, overflow: "hidden" }}>
                <div style={{ width: `${ft.pct}%`, height: "100%", borderRadius: 4, background: d.color }} />
              </div>
            </div>
          ))}
          <div style={{ padding: "8px 10px", background: "#0a0e1a", borderRadius: 6, fontSize: 10, fontFamily: "monospace", color: "#7a9bbf" }}>
        ...
        <div className="card">
          ...
          {d.notable.map((e) => (
            <div key={e.year + e.event} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #1e2d45" }}>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontFamily: "monospace", fontSize: 11, color: d.color, fontWeight: 700 }}>{e.year}</span>
                  <span style={{ fontSize: 12, fontWeight: 600 }}>{e.event}</span>
                </div>
                <div style={{ fontSize: 10, color: "#7a9bbf", fontFamily: "monospace" }}>
                  💀 {e.deaths} · 👥 {e.affected}
                </div>
              </div>
              <span className={`badge badge-${e.sev}`} style={{ fontSize: 9 }}>{e.sev.toUpperCase()}</span>
            </div>
          ))}
        </div>
      </div>
